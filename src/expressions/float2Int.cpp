...
llvm::Value* ASTExpressionFloat2Int::Compile(llvm::Module& mod, llvm::IRBuilder<>& builder, ASTFunction* func)
{
    // Make sure operand is valid float type.
    if (!operand->ReturnType(func)->Equals(&VarTypeSimple::RealType))
    ...
    return builder.CreateFPToSI(operand->CompileRValue(mod, builder, func), VarTypeSimple::IntType.GetLLVMType(builder.getContext()));
}
