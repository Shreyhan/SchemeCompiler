...
llvm::Value* ASTExpressionCall::Compile(llvm::Module& mod, llvm::IRBuilder<>& builder, ASTFunction* func)
{
    ...
    int maxParameters = minParameters;
    ...
    for (int i = 0; i < minParameters; i++) // We only need to check the non-variadic parameters.
    {
        ...
        if (!funcType->parameterTypes[i]->Equals(argReturnType.get())) // The value types are not equal, try seeing if it's a Real we need to cast an int to.
        {
            if (!(funcType->parameterTypes[i]->Equals(&VarTypeSimple::RealType) && argReturnType->Equals(&VarTypeSimple::IntType))) // Uncastable.
                throw std::runtime_error("ERROR: Invalid type passed to function!");
            else
            {
                auto tmp = std::move(arguments[i]);
                arguments[i] = std::make_unique<ASTExpressionInt2Float>(ast, std::move(tmp)); // Cast the int expression to float.
            }
        }
    }

    // Compile all the values and then perform a call. It's important for everything to be R-Values.
    llvm::Value* calleeVal = callee->CompileRValue(mod, builder, func);
    std::vector<llvm::Value*> argumentVals;
    for (auto& arg : arguments)
    {
        argumentVals.push_back(arg->CompileRValue(mod, builder, func));
    }
