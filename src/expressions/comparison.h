...
public:
    ...
    // a1: Left operand.
    // a2: Right operand.
    ASTExpressionComparison(AST& ast, ASTExpressionComparisonType type, std::unique_ptr<ASTExpression> a1, std::unique_ptr<ASTExpression> a2) : ASTExpression(ast), type(type), a1(std::move(a1)), a2(std::move(a2)) {}

    // Create a new comparison expression.
    // type: Type of comparison to do.
    // a1: Left operand.
    // a2: Right operand.
    static auto Create(AST& ast, ASTExpressionComparisonType type, std::unique_ptr<ASTExpression> a1, std::unique_ptr<ASTExpression> a2)
    {
        return std::make_unique<ASTExpressionComparison>(ast, type, std::move(a1), std::move(a2));
    }
