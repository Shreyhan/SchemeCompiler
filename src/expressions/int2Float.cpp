...
{
    return VarTypeSimple::RealType.Copy(); // Of course Int2Float returns a float what else would it.
}
...
llvm::Value* ASTExpressionInt2Float::Compile(llvm::Module& mod, llvm::IRBuilder<>& builder, ASTFunction* func)
{
    ...
    return builder.CreateSIToFP(operand->CompileRValue(mod, builder, func), VarTypeSimple::RealType.GetLLVMType(builder.getContext()));
}
